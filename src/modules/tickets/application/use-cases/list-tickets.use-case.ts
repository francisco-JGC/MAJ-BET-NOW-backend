...
const MAX_LIMIT = 100;
// Safety cap for the non-paginated (mobile) path — same as the old behavior.
const UNPAGINATED_CAP = 100_000;
...
export class ListTickets implements UseCase<ListTicketsInput, ListTicketsOutput> {
  ...
  async execute(input: ListTicketsInput): Promise<ListTicketsOutput> {
    // `limit` present → web pagination path (returns one page + SQL stats).
    // `limit` absent  → mobile / legacy path (returns all tickets, old behavior).
    const paginate = input.limit !== undefined;
    const pageLimit = paginate ? Math.min(input.limit!, MAX_LIMIT) : UNPAGINATED_CAP;
    const pageOffset = paginate ? (input.page ?? 0) * pageLimit : 0;
    ...
    // ── Paginated path (web) ────────────────────────────────────────────────
    // Run SQL stats over the full filtered set in parallel with the page fetch.
    // Only `pageLimit` items come to Node.js for evaluation.
    if (paginate) {
      ...
      const [statsRows, tickets] = await Promise.all([
        ...
        this.tickets.findMany(findManyArgs),
      ]);
      ...
      const items = await this.evaluateTickets(tickets);
      return {
        items,
        total: Number(stats.total_count),
        totalBilled: Number(stats.total_billed),
    ...
    // ── Non-paginated path (mobile / legacy) ────────────────────────────────
    // Fetch all tickets (bounded to UNPAGINATED_CAP) and evaluate in Node.js.
    // Stats are derived from the returned items — no extra SQL round-trip.
    const tickets = await this.tickets.findMany(findManyArgs);
    ...
    for (const item of items) {
      if (item.status === TicketStatus.VALID) {
        totalBilled += item.total;
        totalWonPrize += item.wonPrize;
      }
    }
  ...
  private async evaluateTickets(tickets: Ticket[]): Promise<TicketOutput[]> {
